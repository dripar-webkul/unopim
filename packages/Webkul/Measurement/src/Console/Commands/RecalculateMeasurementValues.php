...
class RecalculateMeasurementValues extends Command
{
     ...
     * @var string
     */
    protected $signature = 'measurement:recalculate
     ...
     * @var string
     */
    protected $description = 'Rebuild the stored base value of every product measurement from the current family definitions';
    ...
    public function handle(
        ...
        if (empty($this->measurementAttributes)) {
        ...
        $productRepository->getModel()->newQuery()
            ...
            ->chunkById($chunk, function ($products) use (&$scanned, &$changed, &$valuesChanged, $helper, $dryRun) {
    ...
    protected function rebuildScope(array &$scopedValues, MeasurementHelper $helper): int
    {
        ...
        foreach ($scopedValues as $attributeCode => $value) {
            ...
            if (! $attribute || ! is_array($value) || ! isset($value['unit'])) {
                continue;
            }
